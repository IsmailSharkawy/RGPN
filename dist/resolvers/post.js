...
let PostResolver = class PostResolver {
    ...
    post(_id, ctx) {
        return ctx.em.findOne(Post_1.Post, { _id });
    }
    ...
    async editPost(_id, title, ctx) {
        const post = await ctx.em.findOne(Post_1.Post, { _id });
        if (!post) {
            return null;
        }
    ...
    async deletePost(_id, ctx) {
        try {
            await ctx.em.nativeDelete(Post_1.Post, { _id });
        }
        catch (error) {
            return false;
        }
